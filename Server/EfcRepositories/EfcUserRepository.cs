...
public class EfcUserRepository : IUserRepository
{
    ...
    public async Task DeleteAsync(int id)
    {
        ...
        if (existing == null)
        {
            throw new InvalidOperationException($"Bruger med id {id} ikke fundet !");
        }
    ...
    {
        User? existing =
            await ctx.Users.SingleOrDefaultAsync(u => u.Name == username);
        if (existing == null)
        {
            throw new InvalidOperationException(
                $"Bruger med id {username} ikke fundet !");
        }
